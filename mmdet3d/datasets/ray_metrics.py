...
def calc_metrics(pcd_pred_list, pcd_gt_list):
    ...
    ave_tp_list = ave[1][:-1] / ave_count[1][:-1]  # use threshold = 2
    ave_gt_ray_list=ave_all[1][:-1]/ave_count_all[1][:-1]
    
    return iou_list, ave_tp_list,ave_gt_ray_list#,depth_sigloss
...
def main(sem_pred_list, sem_gt_list, flow_pred_list, flow_gt_list, lidar_origin_list,density_pred_list,log_dirs='openocc_training_eval_results'):
    ...
    iou_list, ave_tp_list,ave_gt_ray_list= calc_metrics(pcd_pred_list, pcd_gt_list)

    ave_list=calc_whole_flow(sem_gt_list,flow_pred_list, flow_gt_list)
    
    table = PrettyTable([
        'Class Names',
        'IoU@1', 'IoU@2', 'IoU@4','AVE@TP','AVE&GT_RAY','AVE'
    ])
    ...
    for i in range(len(occ_class_names) - 1):
        table.add_row([
            occ_class_names[i],
            iou_list[0][i], iou_list[1][i], iou_list[2][i], ave_tp_list[i],ave_gt_ray_list[i],ave_list[i]
    ...
    table.add_row([
        ...
        np.nanmean(ave_tp_list),
        np.nanmean(ave_gt_ray_list),
        np.nanmean(ave_list)

    ])

    miou = np.nanmean(iou_list[:3])
    mave = np.nanmean(ave_tp_list)
