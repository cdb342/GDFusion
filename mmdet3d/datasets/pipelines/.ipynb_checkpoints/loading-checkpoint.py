...
def get_origin( info,scene_frame):
    ...
    # scene_token = get_scene_token(info)
    # scene_frame = info['scene_frames'][scene_token]
    ...
    # if len(output_origin_list) < 8:
    #     print('Warning: less than 8 origins', len(output_origin_list),11111111111111)
    #     len_list = len(output_origin_list)
    #     for _ in range(8-len_list):
    #         output_origin_list.append(output_origin_list[-1])
    #     print('Warning: less than 8 origins', len(output_origin_list),222222222222222)

    output_origin_tensor = torch.from_numpy(np.stack(output_origin_list))  # [T, 3]
    # print(self.data_infos[idx].keys(),1111111111)
...
class LoadOccFlowGTFromFile(object):
    ...
    def __init__(self, occupancy_path='/mount/dnn_data/occupancy_2023/gts',
                ...
                occ_ray_mask2=False,
                occ_ray_mask3=False,
                load_render_depth=False,
                occ_ray_mask2flow_only=False,
                load_swin_pred=False,
        ...
        self.occ_ray_mask2=occ_ray_mask2
        self.occ_ray_mask3=occ_ray_mask3
        self.load_render_depth=load_render_depth
        self.occ_ray_mask2flow_only=occ_ray_mask2flow_only
        self.load_swin_pred=load_swin_pred
    ...
    def __call__(self, results):
        ...
        if self.occ_ray_mask:
            ...
            if self.occ_ray_mask2:
                ray_mask=metas['ray_mask2']
            else:
                ray_mask=metas['ray_mask']
            ray_mask=torch.tensor(ray_mask)
            if self.occ_ray_mask3:
                semantic_mask=occupancy!=self.num_classes-1
                ray_mask=ray_mask|semantic_mask
            if not self.occ_ray_mask2flow_only:
                occupancy[~ray_mask]=255
            # occ_flow[~ray_mask]=255
        # visible_mask = torch.tensor(data[self.mask])
        # visible_mask_lidar = data['mask_lidar']
        ######
        # occupancy_original = occupancy.clone()
        #####
        # print(self.ignore_nonvisible,11111111111111)
        ...
        if results['rotate_bda'] != 0:
            ...
            ######################
            occ_flow = occ_flow.permute(2, 0, 1,3)
            occ_flow = rotate(occ_flow, -results['rotate_bda'], fill=255).permute(1, 2, 0,3)
            ######################
            ######################
            if self.occ_ray_mask:
                occupancy_original = occupancy_original.permute(2, 0, 1)
        ...
        # for i in range (7):
        #     mask=occupancy==i+1
        #     mask2=  torch.tensor(data['semantics'])==i
        #     # occ_flow= torch.tensor(data['flow'])
        #     print(occ_flow.shape,mask.shape,torch.tensor(data['flow']).shape,mask2.shape,mask.sum(),i,occ_flow[mask].shape,torch.tensor(data['flow'])[mask2].shape,1000000000000)
        #     print(occ_flow[mask].mean(0),torch.tensor(data['flow'])[mask2].mean(0),occ_flow[mask].std(0),torch.tensor(data['flow'])[mask2].std(0),1111111111111111)

        #     print(occ_flow[mask].max(0),torch.tensor(data['flow'])[mask2].max(0),occ_flow[mask].min(0),torch.tensor(data['flow'])[mask2].min(0),22222222222222221)
        if self.occ_ray_mask:
        ...
        if self.load_render_depth:
            render_depth_path=occupancy_file_path.replace('openocc_v2','openocc_v2_pred_dists')
            render_depth=np.load(render_depth_path)['pred_dists']
            results['gt_render_depth'] = DataContainer(torch.tensor(render_depth).reshape(-1))
            gt_pred_pcds_t_path=occupancy_file_path.replace('openocc_v2','openocc_v2_pred_pcds_t')
            gt_pred_pcds_t=np.load(gt_pred_pcds_t_path)['pred_pcds_t']
            results['gt_pred_pcds_t']=DataContainer(torch.tensor(gt_pred_pcds_t))
        if self.load_swin_pred:
            swin_pred_path = occupancy_file_path.split('openocc_v2/')[-1]
            swin_pred_path = swin_pred_path.replace('/labels.npz', '.npz')
            swin_pred_path=swin_pred_path.replace('.npz','/openocc_v2_pred_pcds_t.npz')
            swin_pred_path=os.path.join('occupancy_vis/swin_base_80088/occupancy_pred',swin_pred_path)

            if os.path.exists(swin_pred_path):
                swin_pred_pcds_t=np.load(swin_pred_path)['pred_pcds_t']
            else:
                swin_pred_pcds_t=np.nan
            results['gt_swin_pred_pcds_t']=DataContainer(torch.tensor(swin_pred_pcds_t))
            if self.flow_prev2curr:
                swin_pred_path = occupancy_file_path.split('openocc_v2/')[-1]
                swin_pred_path = swin_pred_path.replace('/labels.npz', '.npz')
                swin_pred_path=os.path.join('occupancy_vis/swin_base_80088/occupancy_pred',swin_pred_path)
                if os.path.exists(swin_pred_path):
                    swin_pred_occ=np.load(swin_pred_path)['arr_0']
                    swin_pred_occ=torch.tensor(swin_pred_occ)
                    swin_pred_occ=self.trans_occ(swin_pred_occ,results)

                else:
                    swin_pred_occ=torch.tensor(np.nan)

                prev_gt_occ_path=results['prev_occ_gt_path']
                if prev_gt_occ_path is not None:
                    prev_gt_occ_path=prev_gt_occ_path.replace('gts','openocc_v2')

                    prev_swin_pred_path=prev_gt_occ_path.split('openocc_v2/')[-1]
                    # print(prev_swin_pred_path,11222222222222222222222111)
                    prev_swin_pred_path=prev_swin_pred_path+ '.npz'
                    # print(prev_swin_pred_path,1133333333333333333333333333311)
                    prev_swin_pred_path=os.path.join('occupancy_vis/swin_base_80088/occupancy_pred',prev_swin_pred_path)
                    # print(prev_swin_pred_path,11111111111111111111111)
                    if os.path.exists(prev_swin_pred_path):
                        prev_swin_pred_occ=np.load(prev_swin_pred_path)['arr_0']
                        prev_swin_pred_occ=torch.tensor(prev_swin_pred_occ)
                        prev_swin_pred_occ=self.trans_occ(prev_swin_pred_occ,results)
                    else:
                        prev_swin_pred_occ=torch.tensor(np.nan)
                        # print(prev_swin_pred_path,22222222)
                else:
                    prev_swin_pred_occ=torch.tensor(np.nan)
                results['gt_swin_pred_occ']=DataContainer(swin_pred_occ)
                results['gt_prev_swin_pred_occ']=DataContainer(prev_swin_pred_occ)
        if self.flow_curr2future:
            ...
            # print(results.keys(),22222222222222,results['curr'].keys())
            # results['ray_origin']=get_origin( results['curr'],results['scene_frame'])
        # print(results['gt_occupancy'].max(),results['gt_occupancy_ori'].max(),222222222222222222222)
...
@PIPELINES.register_module()
class LoadMultiViewImageFromFiles_FBBEV(object):
    """Load multi channel images from a list of separate channel files.
...
@PIPELINES.register_module()
class LoadImageFromFileMono3D_FBBEV(object):
...
@PIPELINES.register_module()
class LoadPointsFromMultiSweeps_FBBEV(object):
...
@PIPELINES.register_module()
class PointSegClassMapping_FBBEV(object):
...
@PIPELINES.register_module()
class NormalizePointsColor_FBBEV(object):
...
@PIPELINES.register_module()
class LoadPointsFromFile_FBBEV(object):
...
@PIPELINES.register_module()
class LoadPointsFromDict_FBBEV(LoadPointsFromFile_FBBEV):
...
@PIPELINES.register_module()
class LoadAnnotations3D_FBBEV(LoadAnnotations):
    """Load Annotations3D.
...
@PIPELINES.register_module()
class PointToMultiViewDepth_FBBEV(object):
...
@PIPELINES.register_module()
class PrepareImageInputs_FBBEV(object):
    """Load multi channel images from a list of separate channel files.
    ...
    def __init__(
        ...
        load_depth_anything=False,
        load_semantic_anything=False,
    ):
        ...
        self.load_depth_anything=load_depth_anything
        self.load_semantic_anything=load_semantic_anything
        if load_semantic_anything:
    ...
    def get_inputs(self, results, scale=None):
        # print(results.keys(),2222222222222222222222222222)
        imgs = []
        rots = []
        trans = []
        intrins = []
...
#########################
            if self.load_depth_anything:
                # print(TF.to_tensor(img).shape,100000000000000000000000)
                depth_anything_name = filename.replace('samples', 'depth_anything')
                depth_anything_name = depth_anything_name.replace('jpg', 'npy')
                depth_anything_map = np.load(depth_anything_name)
                depth_anything_map = torch.from_numpy(depth_anything_map)
                # depth_anything_map = depth_anything_map.permute(0, 2, 1)
                # print(depth_anything_map.shape, img.size, resize_dims, crop, flip, rotate,9999999999999999)
                depth_anything_map = F.interpolate(
                    depth_anything_map.unsqueeze(0),
                    size=img.size,
                    mode='bilinear',
                    align_corners=True).squeeze(0).squeeze(0)
                # print(depth_anything_map.shape, img.size, resize_dims, crop, flip, rotate,222222222222222222)
                depth_anything_map=TF.to_pil_image(depth_anything_map)
                depth_anything_map=self.img_transform_core(
                    depth_anything_map,
                    resize_dims=resize_dims,
                    crop=crop,
                    flip=flip,
                    rotate=rotate)
                depth_anything_map =TF.to_tensor(depth_anything_map)
                # print(depth_anything_map.shape, img.size, resize_dims, crop, flip, rotate,3333333333333333333333)
                depth_anything_maps.append(depth_anything_map)
            if self.load_semantic_anything:
                semantic_anything_name = filename.replace('samples', 'nuscenes_semantic/samples')
                semantic_anything_name = semantic_anything_name.replace('.jpg', '_mask.bin')
                semantic_anything_map = np.fromfile(semantic_anything_name, dtype=np.int8).reshape(900, 1600)
                semantic_anything_map = self.semantic_map[semantic_anything_map]
                semantic_anything_map = torch.from_numpy(semantic_anything_map)
                semantic_anything_map=F.interpolate(
                    semantic_anything_map.float().unsqueeze(0).unsqueeze(0),
                    size=resize_dims[::-1],
                    mode='nearest').squeeze(0).squeeze(0)
                semantic_anything_map=semantic_anything_map.numpy()



                # print(semantic_anything_map.shape, semantic_anything_map.max(),semantic_anything_map.min(),semantic_anything_map[:10,:10],2222222222222222222)
                # semantic_anything_map.flags.writeable = True
                semantic_anything_map = Image.fromarray(semantic_anything_map)
                # print(np.array(semantic_anything_map).shape,np.array(semantic_anything_map).max(),np.array(semantic_anything_map).min(),np.array(semantic_anything_map)[:10,:10],111111111111111111111)

                # semantic_anything_map=self.img_transform_core(
                #     semantic_anything_map,
                #     resize_dims=resize_dims,
                #     crop=crop,
                #     flip=flip,
                #     rotate=rotate)
                ...
                # # adjust image
                # img = img.resize(resize_dims)
                semantic_anything_map = semantic_anything_map.crop(crop)
                if flip:
                    semantic_anything_map = semantic_anything_map.transpose(method=Image.FLIP_LEFT_RIGHT)
                semantic_anything_map = semantic_anything_map.rotate(rotate)
                # return img
                semantic_anything_map = np.array(semantic_anything_map)
                
                semantic_anything_map = torch.from_numpy(semantic_anything_map)
                semantic_anything_maps.append(semantic_anything_map)
                # print(semantic_anything_map.shape, img.size, resize_dims, crop, flip, rotate,3333333333333333333333)
...
#######################
            ...
            # print(filename, img.size, resize_dims, crop, flip, rotate)
            # import pdb;pdb.set_trace()
            imgs.append(self.normalize_img(img, **self.normalize_cfg))
            # print(imgs[0].shape,55555)
        ...
        if self.sequential:
            ...
            for adj_info in results['adjacent']:
                # adj_post_trans.extend(post_trans[:len(cam_names)])
                # adj_post_rots.extend(post_rots[:len(cam_names)])
                # adj_intrins.extend(intrins[:len(cam_names)])
                ...
                # adj_rots.extend(rots_adj)
                # adj_trans.extend(trans_adj)
                # adj_sensor2sensors.extend(sensor2sensors_adj)
            # adj_post_trans=torch.stack(adj_post_trans)
            # adj_post_rots=torch.stack(adj_post_rots)
            # adj_intrins=torch.stack(adj_intrins)
            adj_sensor2egos=torch.stack(adj_sensor2egos)
            adj_ego2globals=torch.stack(adj_ego2globals)
            # adj_rots=torch.stack(adj_rots)
            # adj_trans=torch.stack(adj_trans)
        ...
        if self.load_depth_anything:
            depth_anything_maps=torch.stack(depth_anything_maps)
            results['depth_anything_map'] = depth_anything_maps
        if self.load_semantic_anything:
            semantic_anything_maps=torch.stack(semantic_anything_maps)
            results['semantic_anything_map'] = semantic_anything_maps
        if self.sequential:
            return (imgs, rots, trans, intrins, post_rots, post_trans), (sensor2egos, ego2globals), (adj_sensor2egos, adj_ego2globals)
        else:
...
@PIPELINES.register_module()
class LoadAnnotationsBEVDepth_FBBEV(object):
