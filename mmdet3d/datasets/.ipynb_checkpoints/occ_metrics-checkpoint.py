...
class Metric_mIoU():
    ...
    def add_batch(self,semantics_pred,semantics_gt,mask_lidar,mask_camera):
        ...
        _,_hist_occ= self.compute_mIoU((masked_semantics_pred!=17).astype(np.float32), (masked_semantics_gt!=17).astype(np.float32), 2)
        self.hist += _hist
    ...
    def count_miou(self):
        ...
        # import pdb;pdb.set_trace()
        occ_class=['free','occupied']
        mIoU_occ = self.per_class_iu(self.hist_occ)
        for ind_class in range(2):
            print(f'===> {occ_class[ind_class]} - IoU = ' + str(round(mIoU_occ[ind_class] * 100, 4)))
