...
def process_one_sample(sem_pred, lidar_rays, output_origin, flow_pred,dvr,occ_pred=None,return_coords=False):
    ...
    for t in range(T): 
        ...
        if flow_pred is None:
            pred_flow = torch.zeros_like(coord_index[:,:2], dtype=torch.float32)
        else:
        ...
        #     print(pred_dist.min(),lidar_origin,11111111111111111)
        pred_pcds_t.append(pred_pcds)
        coords.append(coord_index)

    pred_pcds_t = torch.cat(pred_pcds_t, dim=0)
    if return_coords:
        return pred_pcds_t.numpy(), torch.cat(coords,dim=0).numpy()
    return pred_pcds_t.numpy()
def calc_whole_flow(sem_gt_list,pcd_pred_list, pcd_gt_list):
    ave = np.zeros([ len(occ_class_names)])
    for i, cls in enumerate(occ_class_names):
        if cls not in flow_class_names:
            ave[ i] = np.nan
    # import pdb; pdb.set_trace()
    ave_count = np.zeros([len(occ_class_names)])
    for sem_gt, pcd_pred, pcd_gt in zip(sem_gt_list,pcd_pred_list, pcd_gt_list):
        for i, cls in enumerate(occ_class_names):
            cls_id = occ_class_names.index(cls)
            cls_mask_gt = (sem_gt== cls_id)
            if cls in flow_class_names and cls_mask_gt.sum() > 0:
                gt_flow_i_all=pcd_gt[cls_mask_gt]
                pred_flow_i_all=pcd_pred[cls_mask_gt]
                # import pdb; pdb.set_trace()
                flow_error_all=np.linalg.norm(gt_flow_i_all-pred_flow_i_all,axis=1)
                ave[i]+=np.sum(flow_error_all)
                ave_count[i]+=flow_error_all.shape[0]
    ave_count=ave[:-1]/ave_count[:-1]
    return ave_count
...
def calc_metrics(pcd_pred_list, pcd_gt_list):
    ...
    #############
    ave_all = np.zeros([len(thresholds), len(occ_class_names)])
    for i, cls in enumerate(occ_class_names):
        if cls not in flow_class_names:
            ave[:, i] = np.nan

    ave_count_all = np.zeros([len(thresholds), len(occ_class_names)])
    ##########
    ...
    for pcd_pred, pcd_gt in zip(pcd_pred_list, pcd_gt_list):
        for j, threshold in enumerate(thresholds):
            ...
            for i, cls in enumerate(occ_class_names):
                ...
                #############
                if cls in flow_class_names and cls_mask_gt.sum() > 0:
                    gt_flow_i_all=pcd_gt[cls_mask_gt,2:4]
                    pred_flow_i_all=pcd_pred[cls_mask_gt,2:4]
                    # import pdb; pdb.set_trace()
                    flow_error_all=np.linalg.norm(gt_flow_i_all-pred_flow_i_all,axis=1)
                    ave_all[j][i]+=np.sum(flow_error_all)
                    ave_count_all[j][i]+=flow_error_all.shape[0]



            #
        # depth_sigloss=SigLoss()(torch.tensor(torch.tensor(depth_pred),dtype=torch.float32),torch.tensor(torch.tensor(depth_gt),dtype=torch.float32))
    ...
    ave_list_all=ave_all[1][:-1]/ave_count_all[1][:-1]
    
    return iou_list, ave_list,ave_list_all#,depth_sigloss


def main(sem_pred_list, sem_gt_list, flow_pred_list, flow_gt_list, lidar_origin_list,density_pred_list,log_dirs='openocc_training_eval_results'):
    ...
    pcd_pred_list_all, pcd_gt_list_all = [], []
    for sem_pred, sem_gt, flow_pred, flow_gt, lidar_origins,density_pred in tqdm(zip(sem_pred_list, sem_gt_list, flow_pred_list, flow_gt_list, lidar_origin_list,density_pred_list)):
        ...
        #     density_pred = np.reshape(density_pred, [200, 200, 16])

        pcd_pred = process_one_sample(sem_pred, lidar_rays, lidar_origins, flow_pred,dvr,density_pred)
        pcd_gt = process_one_sample(sem_gt, lidar_rays, lidar_origins, flow_gt,dvr)

        # evalute on all rays
        pcd_pred_list_all.append(pcd_pred)
        pcd_gt_list_all.append(pcd_gt)
    ...
    iou_list, ave_list,ave_list_all= calc_metrics(pcd_pred_list, pcd_gt_list)
    # iou_list, ave_list,depth_sigloss = calc_metrics(pcd_pred_list, pcd_gt_list)
    # iou_list_all, ave_list_all,depth_sigloss_all = calc_metrics(pcd_pred_list_all, pcd_gt_list_all)

    whole_flow_error=calc_whole_flow(sem_gt_list,flow_pred_list, flow_gt_list)
    
    table = PrettyTable([
        'Class Names',
        'IoU@1', 'IoU@2', 'IoU@4','AVE','AVE_all_gt_ray','AVE_whole'
    ])
    ...
    for i in range(len(occ_class_names) - 1):
        table.add_row([
            occ_class_names[i],
            iou_list[0][i], iou_list[1][i], iou_list[2][i], ave_list[i],ave_list_all[i],whole_flow_error[i]
    ...
    table.add_row([
        ...
        # np.nanmean(iou_list[3]),
        np.nanmean(ave_list),
        np.nanmean(ave_list_all),
        np.nanmean(whole_flow_error)

    ])
    print(' --- IoU and AVE metrics ---')
    # print('--- IoU@1 ---')
    # for i in range(len(occ_class_names)-1):
    #     print(occ_class_names[i],':',iou_list[0][i])
    # print('--- IoU@2 ---')
    # for i in range(len(occ_class_names)-1):
    #     print(occ_class_names[i],':',iou_list[1][i])
    # print('--- IoU@4 ---')
    # for i in range(len(occ_class_names)-1):
    #     print(occ_class_names[i],':',iou_list[2][i])
    print(table)


    miou = np.nanmean(iou_list[:3])
    ...
    # print(' --- Depth SigLoss:', depth_sigloss)
    # print(' --- Depth Sigloss all:',depth_sigloss_all)


    from mmdet3d.utils import collect_env, get_root_logger
    # log_dir='openocc_training_eval_results'
    import os
    ...
    # logger.info(' --- Depth SigLoss:'+str(depth_sigloss.cpu().numpy()      ))
    # logger.info(' --- Depth Sigloss all:'+str(depth_sigloss_all.cpu().numpy()))
